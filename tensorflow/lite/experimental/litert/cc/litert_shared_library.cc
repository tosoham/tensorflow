  ...
  switch (handle_kind) {
    case HandleKind::kInvalid:
      return Error(
          ...
          absl::StrFormat("Could not load shared library %s: invalid handle "
                          "kind. The library may have already been closed.",
                          lib.path_));
...
std::ostream& operator<<(std::ostream& os, const SharedLibrary& lib) {
  ...
  os << absl::StreamFormat("%s", kHeader);
  if (lib.handle_ == nullptr) {
    os << "Handle is nullptr\n";
    os << absl::StreamFormat("%s", kFooter);
    return os;
  }

#if LITERT_IMPLEMENT_SHARED_LIBRARY_INFO
  Lmid_t dl_ns_idx;
  if (0 != ::dlinfo(lib.handle_, RTLD_DI_LMID, &dl_ns_idx)) {
    return os;
  }

  link_map* lm;
  if (0 != ::dlinfo(lib.handle_, RTLD_DI_LINKMAP, &lm)) {
    return os;
  }
  ...
  for (const auto* forward = lm->l_next; forward != nullptr;
  ...
  for (const auto* backward = lm->l_prev; backward != nullptr;
